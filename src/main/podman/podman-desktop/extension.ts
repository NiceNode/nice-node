/**
 * Loops over current podman machines, creates socket path based on OS,
 * establishes socket connection to podman machines
 * Defines MachineJSON and MachineInfo
 */
/** ********************************************************************
...
type StatusHandler = (
  name: string,
  event: extensionApi.ProviderConnectionStatus
) => void;
...
const podmanMachineSocketsSymlinkDirectoryMac = path.resolve(
  os.homedir(),
  '.podman'
);
...
const podmanMachinesStatuses = new Map<
  string,
  extensionApi.ProviderConnectionStatus
>();
...
async function updateMachines(provider: extensionApi.Provider): Promise<void> {
  ...
  const machineListOutput = await execPromise(getPodmanCli(), [
    'machine',
    'list',
    '--format',
    'json',
  ...
  machines.forEach((machine) => {
    ...
    let status: extensionApi.ProviderConnectionStatus = running
      ? 'started'
      : 'stopped';
  ...
  await Promise.all(
    connectionsToCreate.map(async (machineName) => {
      ...
      try {
        ...
        console.debug(
          'Podman extension:',
          'Failed to read socketPath from machine inspect'
        );
      }
      if (!socketPath) {
        if (isMac()) {
      ...
      await registerProviderFor(
        provider,
        podmanMachinesInfo.get(machineName),
        socketPath
      );
    })
  );
  ...
  if (machines.length === 0) {
    ...
    const atLeastOneMachineRunning = machines.some(
      (machine) => machine.Running
    );
    const atLeastOneMachineStarting = machines.some(
...
function calcWinPipeName(machineName: string): string {
  const name = machineName.startsWith('podman')
    ? machineName
    : `podman-${machineName}`;
  return `//./pipe/${name}`;
}
...
async function initDefaultLinux(provider: extensionApi.Provider) {
  ...
  const disposable = provider.registerContainerProviderConnection(
...
async function registerProviderFor(
  ...
  const lifecycle: extensionApi.ProviderConnectionLifecycle = {
    ...
    stop: async (context): Promise<void> => {
      await execPromise(getPodmanCli(), ['machine', 'stop', machineInfo.name], {
        logger: context.log,
      });
      provider.updateStatus('ready');
    },
  ...
  const containerConfiguration = extensionApi.configuration.getConfiguration(
    'podman',
    containerProviderConnection
  );
...
export async function activate(
  ...
  disguisedPodmanSocketWatcher = setupDisguisedPodmanSocketWatcher(
    provider,
    getSocketPath()
  );
  ...
  if (isMac() || isWindows()) {
    ...
    provider.registerAutostart({
      start: async (logger: extensionApi.Logger) => {
        ...
        const isRunningMachine = Array.from(
          podmanMachinesStatuses.values()
        ).find(
          (connectionStatus) =>
            connectionStatus === 'started' || connectionStatus === 'starting'
        );
        if (isRunningMachine) {
          console.log(
            'Podman extension:',
            'Do not start a machine as there is already one starting or started'
          );
          return;
        }
        ...
        if (machines.length > 0) {
          ...
          await execPromise(getPodmanCli(), ['machine', 'start', machineName], {
            logger,
          });
        }
      },
  ...
  if (isMac() || isWindows()) {
    ...
    const createFunction = async (
      ...
      if (params['podman.factory.machine.image-path']) {
        ...
        const assetImagePath = path.resolve(
          getAssetsFolder(),
          `podman-image${suffix}`
        );
...
function setupDisguisedPodmanSocketWatcher(
  ...
  if (isLinux) {
    ...
    socketWatcher = extensionApi.fs.createFileSystemWatcher(
      path.dirname(socketFile)
    );
  }
...
async function checkDisguisedPodmanSocket(provider: extensionApi.Provider) {
  ...
  const retrievedWarnings = isDisguisedPodmanSocket
    ? []
    : [getDisguisedPodmanInformation()];
