...
export const runCommand = async (command: string, log?: boolean) => {
	...
	logger.info(`Running podman ${command}`);
	let dockerCommand = command.replaceAll(
		"host.containers.internal",
		"127.0.0.1",
	);
	// use Windows default cmd as shell for easier path escaping compatibility
	// over powershell.
	const data = await podmanExecPromise(`docker ${dockerCommand}`, [], {
		shell: true,
	});
	if (log) {
...
export const sendLogsToUI = (node: Node) => {
	...
	const childProcess = spawn(
		`docker logs --follow --timestamps --tail 100 ${containerId}`,
		watchInput,
		spawnOptions,
	);
	// const childProcess = spawn(
...
export const createRunCommand = (node: Node): string => {
	...
	const podmanCommand = `run --network host -q -d ${envConfigInput} ${directUserEnvConfigInput} --name ${containerName} ${finalPodmanInput} ${imageNameWithTag} ${nodeInput}`;
