...
export const checkForUpdates = async (emit: (e: any, args?: any) => void) => {
  ...
  if (currentVersion < latestVersion) {
    ...
    if (localPackageType === null) {
      ...
      if (localPackageType === null) {
        emit(
          'error',
          new Error(
            'Local supported package type not found. Only deb and rpm currently supported.',
          ),
        );
        return;
      }
    ...
    if (!downloadUrl || !fileName) {
      emit(
        'error',
        new Error('No github release found matching this architecture'),
      );
      return;
    }
...
export const quitAndInstall = async () => {
  ...
  if (localPackageManager === null) {
    ...
    if (localPackageManager === null) {
      throw new Error(
        'Local supported package manager not found. Only dpkg, yum, dnf and zypper currently supported.',
      );
    }
  }
